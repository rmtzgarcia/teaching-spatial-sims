"""
Plant Community Simulation (Gaussian competition kernel)
===========================================================
...
Section 2.1, with a Gaussian competition kernel.
...
birth-death process (Gillespie's stochastic simulation algorithm):
  ...
  - each individual dies at rate d + g * sum_j phi_C(x_i - x_j) (Eq. 2.2),
    where phi_C is a Gaussian competition kernel of range `r_c`, normalized
    to integrate to 1 over the plane.
...
the simulated patch, and a pair-correlation-function C(r) diagnostic.
...
DEFAULT_RC = 2.0    # competition range (slider default; paper sweeps 0-4)
DEFAULT_S = 0.3     # dispersal range (slider default; paper sweeps 0-1.25)
...
def competition_kernel(dist, r_c):
    """Gaussian competition kernel (Eq. 2.3, first case), normalized to
    integrate to 1 over the plane: phi_C(r) = exp(-r^2/(2 r_c^2)) / (2 pi r_c^2).
    """
    return np.exp(-(dist ** 2) / (2 * r_c ** 2)) / (2 * np.pi * r_c ** 2)
