...
-- FLAT-TERRAIN VARIANT for classroom use.

This is adapted from the original interactive script (which included a
downhill water-advection term v, producing the banded "tiger bush" patterns
seen on slopes). Here the terrain is flat: v = 0 everywhere, and there is no
advection slider. This is deliberate for the practicum this script supports:
on flat ground the classic banded pattern cannot form, but the *same*
short-range-activation / long-range-inhibition logic used in the morning's
IBM/PDE lecture still produces spatial self-organization -- here as roughly
isotropic patches/spots rather than oriented bands, once water diffuses much
farther than plants do (Dw >> Dn). If you ever want the sloped/banded version
back, reintroduce a v * dW/dX upwind term into dW/dt.

Model (nondimensionalized water W and plant biomass N, flat domain X,Y with
periodic boundary conditions):

    dW/dt = a - W - W N^2                + Dw * Laplacian(W)
    ...
    Dn : plant (biomass) diffusion coefficient -- SHORT-range: plants spread
         locally (seeds, clonal growth) and locally improve their own water
         infiltration (self-activation).
    Dw : water diffusion coefficient -- LONG-range: surface/subsurface water
         redistributes over larger distances, so a patch of plants depletes
         water for a wide neighborhood (long-range inhibition/competition).
...
Non-spatial (ODE) equilibria, used for the bifurcation diagram on the right:
    ...
    trivial (bare-soil) state exists; above a_c bare soil coexists with a
    stable vegetated state (upper branch) separated by an unstable state
    (lower branch). This fold is what drives the tipping-point/hysteresis
    experiment in the practicum, and it does not depend on Dn, Dw (diffusion
    does not move the equilibria, only whether they are spatially uniform or
    patterned).

    Turing (patterning) condition for this flat model, linearizing around the
    upper branch (W*, N*): patterns appear once
        Dw * m  >  Dn * (1 + N*^2)
    (a necessary condition; see also the quadratic discriminant condition in
    standard Turing analysis). Since N* grows with rainfall a, patterning is
    *easier* to trigger at lower a (smaller N*) -- worth pointing out to
    students who might expect "more rain = more pattern".

GUI: built with Tkinter (ttk) + a matplotlib Figure embedded via
FigureCanvasTkAgg.

    LEFT   : sliders for a, m, R, Dn, Dw + Reset/Pause buttons
...
Run with:  python klausmeier_flat.py
...
# ----------------------------------------------------------------------------
Nx, Ny = 41, 41
Lx, Ly = 40.0, 40.0          # domain size (same units as in the paper, ~meters)
dx, dy = Lx / Nx, Ly / Ny
...
# ----------------------------------------------------------------------------
# Default parameters. a, m, R keep patterning OFF by default (safely below the
# Turing condition above) so Phase 2 of the practicum (tipping point /
# hysteresis) starts from a clean, spatially uniform field. Phase 3 asks
# students to lower a and raise Dw to cross into the patterned regime.
# ----------------------------------------------------------------------------
params = dict(a=2.0, m=0.45, R=1.0, Dn=1.0, Dw=2.0)
...
def step():
    """Advance (W, N) by SUBSTEPS_PER_FRAME explicit Euler steps. Flat terrain:
    no advection term -- water and plants only diffuse and react locally."""
    global W, N
    a, m, R, Dn, Dw = (params[k] for k in ("a", "m", "R", "Dn", "Dw"))
    for _ in range(SUBSTEPS_PER_FRAME):
        uptake = W * N ** 2
        dWdt = a - W - uptake + Dw * laplacian(W)
...
# Tkinter application
# ----------------------------------------------------------------------------
class KlausmeierFlatApp:
    SLIDER_SPECS = [
        ("Rainfall  a",           "a",  0.05, 5.0),
        ("Plant mortality  m",    "m",  0.05, 1.5),
        ("Growth yield  R",       "R",  0.1,  3.0),
        ("Plant diffusion  Dn",   "Dn", 0.02, 2.0),
        ("Water diffusion  Dw",   "Dw", 0.0,  40.0),
    ]

    def __init__(self, root):
        self.root = root
        root.title("Klausmeier model -- flat terrain (no slope)")
    ...
    def _build_layout(self):
        ...
        ttk.Label(left, text="Parameters (flat terrain, v = 0)", style="Section.TLabel").pack(
            anchor="w", pady=(0, 8))
        ...
        for label_text, key, vmin, vmax in self.SLIDER_SPECS:
            ...
            if key == "R":
                ttk.Separator(left, orient="horizontal").pack(fill=tk.X, pady=(2, 10))
        ...
        note = ("Reset field: reseeds the domain near the current homogeneous\n"
                "equilibrium (plus small noise) so a fresh pattern can develop.\n\n"
                "Top group (a, m, R) sets the local reaction kinetics and the\n"
                "tipping point a_c = 2m/R seen on the bifurcation diagram below.\n\n"
                "Bottom group (Dn, Dw) sets how far plants vs. water spread.\n"
                "Patches emerge once Dw is large enough relative to Dn -- try\n"
                "lowering a a little, then slowly raising Dw.")
    ...
    def _build_plots(self):
        ...
        self.ax_pattern.set_xlabel("X")
        self.ax_pattern.set_ylabel("Y")
        self.ax_pattern.set_title("Spatial vegetation pattern N(X, Y, t) -- flat terrain")
